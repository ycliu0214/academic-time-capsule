...
my $lib_path = "lib"; 
if(open(INF, "${lib_path}/Au_electrode/2.93_top_left.gjf") != 1){
...
for($i=0; $i<=$#atom; $i++){
	if($i != $LS && $i != $RS && $i != $LH && $i != $RH){
		if($result_z[$i] <= $result_z[$LS] - 50.5 || $result_z[$i] >= $result_z[$RS] + 50.5){
			$special_case=1;
			last;
		}
	}
		...
		for($i=0; $i<=$#fixing_rad; $i++){
			...
			for($j=0; $j<=$#atom; $j++){
				if($j != $LS && $j != $RS && $j != $LH && $j != $RH){
					if($fixing_z[$j] < $fixing_z[$LS] - 50.5 || $fixing_z[$j] > $fixing_z[$RS] + 50.5){
						$fixing_check = 1;
						last;
					}
				}
...
if($electrode_type == 4){
	$left_elec_file = "${lib_path}/Au_electrode/${lattice_length}_top_left.gjf";
	$right_elec_file = "${lib_path}/Au_electrode/${lattice_length}_top_right.gjf";
}
...
print OUTF "#p b3lyp/gen nosymm pop=full geom=connectivity formcheck gfinput pseudo=read\n";
print OUTF "scf=(dsymm,conver=6,nodamp,maxcycle=1000)\n\n";
