...
my $lib_path = "new_lib"; 
if(open(INF, "${lib_path}/Left.gjf") != 1){
...
while(){
	...
	print "You can choose \"2.88\" or \"2.93\" (angstrom)\n";
	$lattice_length = <STDIN>;
	chomp $lattice_length;
	if($lattice_length == 2.93 || $lattice_length == 2.88){ 
		last;
	}
	else{
...
for($i=0; $i<=$#atom; $i++){
	if($i != $LS && $i != $RS && $i != $LH && $i != $RH){
		if($result_z[$i] <= $result_z[$LS] - 4.5 || $result_z[$i] >= $result_z[$RS] + 4.5){
			$special_case=1;
			last;
		}
	}
		...
		for($i=0; $i<=$#fixing_rad; $i++){
			...
			for($j=0; $j<=$#atom; $j++){
				if($j != $LS && $j != $RS && $j != $LH && $j != $RH){
					if($fixing_z[$j] < $fixing_z[$LS] - 4.5 || $fixing_z[$j] > $fixing_z[$RS] + 4.5){
						$fixing_check = 1;
						last;
					}
				}
...
if($electrode_type == 4){
	$left_elec_file = "${lib_path}/Left.gjf";
	$right_elec_file = "${lib_path}/Right.gjf";
}
...
my $shift_x_length = $left_coorx[$#left_coorx] - $result_x[$LS];
my $shift_y_length = $left_coory[$#left_coory] - $result_y[$LS];
my $shift_z_length = $left_coorz[$#left_coorz] - $result_z[$LS];
...
print OUTF "!\n";
print OUTF "    NEMBED1 = 22\n";
print OUTF "    NEMBED2 = 22\n";
print OUTF "!\n";
print OUTF "! *** Number of atoms in each electrode (useful for single-element systems)\n";
print OUTF "!\n";
print OUTF "   NATOMEL1 = 27\n";
print OUTF "   NATOMEL2 = 27\n";
print OUTF "!\n";
print OUTF "! *******************************\n";
