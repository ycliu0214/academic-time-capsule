...
for($i=0; $i<=$#atom; $i++){
	if($i != $LS && $i != $RS && $i != $LH && $i != $RH){
		if($result_z[$i] <= $result_z[$LS] - 0.5 || $result_z[$i] >= $result_z[$RS] + 0.5){
			$special_case=1;
			last;
		}
	}
		...
		for($i=0; $i<=$#fixing_rad; $i++){
			...
			for($j=0; $j<=$#atom; $j++){
				if($j != $LS && $j != $RS && $j != $LH && $j != $RH){
					if($fixing_z[$j] < $fixing_z[$LS] - 0.5 || $fixing_z[$j] > $fixing_z[$RS] + 0.5){
						$fixing_check = 1;
						last;
					}
				}
...
if($electrode_type == 4){
	...
	$right_elec_file = "${lib_path}/Au_electrode/${lattice_length}_bottom_right.gjf";
}
	...
	$left_elec_file = "${lib_path}/Au_electrode/${lattice_length}_bottom_left.gjf";
