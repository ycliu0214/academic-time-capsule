...
fileList = os.listdir('dipoleFiles')
...
for i in fileList:
    with open('dipoleFiles/'+i, 'r') as f:
        for line in f:
            word = line.split()
...
for i in range(0,len(matrix),3):
    try:
        print('{:0.6e}'.format(matrix[i]),end=' ')
        print('{:0.6e}'.format(matrix[i+1]),end=' ')
        print('{:0.6e}'.format(matrix[i+2]))
    except IndexError:
        print('')
